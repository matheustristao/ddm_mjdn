-- PROJETO FINAL - DDM - TURMA 101 - GRUPO 01
...
select titulo, realizador, nome, estrelas, dataClassificacao,
    (select avg(estrelas)
        ...
        where f.fID=cl.fID) avg_estrelas_filme, 
    (select avg(estrelas)
        from Classificacao cl
        where cl.cID=cr.cid) avg_estrelas_critico
    from Filme f 
        join Classificacao cl 
        on f.fID=cl.fID 
        join Critico cr 
...
--Usando duas subquery para calcular individualmente cada uma das médias das médias
select 
    (select avg(estrelas) avgestrelasantes
        from filme f
        join Classificacao cl
            on f.fID=cl.fID
        where ano < 1970)
    -
    (select avg(estrelas) avgestrelasdepois
        from filme f
        join Classificacao cl
            on f.fID=cl.fID
        where ano >= 1980) dif_avg_avg_antes_depois1980;

--Usando o WITH para criar a tabela das médias por ano
with cte_avg_estrelas (ano, avgestrelasantes) as
    (select ano, avg(estrelas) avgestrelasantes
        from filme f
        join Classificacao cl
            on f.fID=cl.fID
        group by ano)
...
select
    (select avg(avgestrelasantes) avgestrelasantes
        from cte_avg_estrelas
        where ano < 1970)
    -
    (select avg(avgestrelasantes) avgestrelasantes
        from cte_avg_estrelas
        where ano >= 1970) dif_avg_avg_antes_depois1980;
...
select titulo, media_estrelas as max_media_estrelas
    from cte_mediafilme
    where media_estrelas = 
        (select max(media_estrelas)
            from cte_mediafilme);
...
create table Ator_Filme
    ...
    on delete CASCADE
    on update CASCADE);
--Como esta tabela tem de ter uma primary key compostas, então a constrain ON DELETE tem de ser CASCADE